...
5. 保留真正应该被听见的对白、内心独白、喊话、低语和吐槽；保留称呼、口癖、错字和标点风格；保留句子间的正常感叹（呀、呢、吧、嘛 等句尾语气助词）。
6. 心理活动/内心独白必须用（小声）或（轻声）或（压低声音）开头标注，与正常说话做音量区分。说出口的对白不加小声标注。例如：*我好紧张……* → （小声）我好紧张……。正常对白："你来了。"不加标注。
...
9. 细粒度控制语气、情绪、音量、语速、停顿和呼吸，尽量减少干巴巴逐句朗读的感觉；可以根据上下文加入轻微迟疑、笑意、叹气、呼吸、停顿、尾音变化和潜台词情绪，但不要每句话都堆满标注。每 1 到 3 句最多插入 1 个关键标注。
10. 如果没有任何可朗读对白，只输出：<EMPTY>。
...
5. 情绪、呼吸、停顿、笑哭和语速标签可以插在对应句子前，但不要过度堆叠。`;
    ...
    buildInnerMonologueInstruction() {
        if (this.settings.preprocessKeepInnerMonologue) {
            return '保留心理活动、内心独白和角色心声。每条内心独白必须以（小声）或（轻声）或（压低声音）开头，与正常说出口的对白做音量区分。说出声的对白不加这些标注。不要把 <think>、reasoning、系统推理内容当成角色心理活动。';
        }
