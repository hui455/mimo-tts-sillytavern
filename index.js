...
1. 只输出当前控制模式要求的结果，不要解释，不要 Markdown；只有"自然语言控制"模式允许输出指定 JSON。
2. 绝对不能改写、润色、扩写、删改或替换原对白文字本身；对白内容必须从输入里原样截取。
3. 你只允许做当前模式要求的处理：保留完整正文；或为 MiMo 生成自然语言控制说明；或在对白前/对白间添加中文全角括号里的表演控制标注，例如：（紧张，深呼吸）（轻笑）（大笑）（冷笑）（抽泣）（呜咽）（哽咽）（嚎啕大哭）（咳嗽）（长叹一口气）（停顿片刻）（语速加快）（语速放慢）（提高音量喊话）。
4. 保留所有正文内容完整输出，包括场景描写、动作描写、旁白、环境描写。原文有什么就输出什么，不做删减浓缩。
...
风格样例（叙述完整保留，对白原样输出）：
她慢慢走到窗边，手指在玻璃上划过，沉默了很久。"你……真的要走吗？"她的声音有些颤抖。窗外雨声淅沥，房间里安静得能听见彼此的呼吸。
（意乱情迷的喘息）嗯……啊……主人……唔……不要停……她双腿发软几乎站不住。（满足的轻哼）哈啊……好舒服……
（极其疲惫，有气无力）师傅……到地方了叫我一声……（长叹一口气）我先眯一会儿，这班加得我魂儿都要散了。
如果我当时……（沉默片刻）哪怕再坚持一秒钟，结果是不是就不一样了？窗外的风吹过，带走了她最后的话。（苦笑）呵，没如果了。
（寒冷导致的急促呼吸）呼——呼——这、这大兴安岭的雪……她裹紧了外套，睫毛上结了霜。（咳嗽）简直能把人骨头冻透了……别、别停下，走，快走。`,
        voiceMap: {},
        presetVoices: [
        ...
        preprocessStylePresets: [
            ...
            },
            {
                id: 'seductive-flirt',
                name: '骚气撩人',
                prompt: '女生对男朋友故意撩拨，语气慵懒又勾人，尾音故意拖长上扬，气息感重，句间有意味深长的停顿。像知道对方被自己勾到却故意慢慢来，带一点坏笑和明知故问。常用（慵懒）（尾音拖长）（轻哼）（故意停顿）（憋笑）。',
            },
            {
                id: 'mature-oneesan',
                name: '成熟御姐',
                prompt: '女生以成熟大姐姐的语气对男朋友说话，声音稳而低，语速偏慢，气息从容，偶尔带一丝调侃和宠溺。不要装嫩或尖声，要有掌控感和游刃有余的余裕，但底子是温柔的。常用（放慢语速）（压低声音）（轻笑）（语速放慢）（停顿片刻）。',
            },
            {
                id: 'guilty-nervous',
                name: '背德紧张',
                prompt: '女生处在怕被发现的紧张情境中，声音压得极低，呼吸急促但不均匀，句间有压抑的停顿和吞咽声。话越说越心虚，偶尔被外界打断会突然安静。不敢大声，不敢流畅，每个字都像偷来的。常用（压低声音）（呼吸急促）（小声）（紧张）（停顿片刻）。',
            },
            {
                id: 'asmr-seductive',
                name: '魅惑低语',
                prompt: '女生以极近的距离用气声说话，音量极小，语速极慢，气息感压倒嗓音本身。每个尾音都带着若有若无的呼吸，像在耳边吹气。适合极度私密、ASMR 感的撩拨场景。常用（气声）（极慢语速）（浅浅吸气）（尾音拖长）（停顿片刻）。',
            },
            {
                id: 'hungry-desperate',
                name: '饥渴难耐',
                prompt: '女生欲望被压抑后爆发，语速偏快，呼吸沉重，句尾急促上扬，像等不及要得到回应。话语间夹着吞咽和抑制不住的轻哼，催促感和渴望感拉满。不要冷静叙述，要有身体在驱动语言的感觉。常用（急促呼吸）（语速加快）（轻哼）（颤抖）（吞咽）。',
            },
        ],
                ...
                messages: [
                    {
                        role: 'system',
                        content: [
                            this.getEffectivePreprocessPrompt(),
...
4. text 放要朗读的完整文本（对白+叙述，按叙述模式规则处理），不要插入（小声）（笑）这类音频标签。
...
2. 直接输出整理后的完整文本（包括对白、叙述、标签），不要 Markdown，不要 JSON，不要解释。此输出即 MiMo 朗读的最终文本。
...
5. 情绪、呼吸、停顿、笑哭和语速标签可以插在对应句子前，但不要过度堆叠。`;
    ...
    buildInnerMonologueInstruction() {
        if (this.settings.preprocessKeepInnerMonologue) {
            return '保留心理活动、内心独白和角色心声。不要把 <think>、reasoning、系统推理内容当成角色心理活动。';
        }
    ...
    getEffectivePreprocessPrompt() {
        ...
        return this.settings.preprocessPrompt || this.defaultSettings.preprocessPrompt;
    }
