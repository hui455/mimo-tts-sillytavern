...
6. 删除或转换纯呻吟/娇喘/浪叫/喘息类拟声词（如：嗯～、啊～、哈啊、咕齁、咿咿、嗯嗯啊啊 等成片无实际语义的叫声），改用 MiMo 括号标注表现情绪，例如转换"嗯❤️～哈啊～"为（满足的轻哼）（意乱情迷的喘息）。禁止 MiMo 直接朗读这些无语义拟声词。
7. 删除插入在中文句子中间的 emoji（❤️♡～等）和非文字符号。
8. 细粒度控制语气、情绪、音量、语速、停顿和呼吸，尽量减少干巴巴逐句朗读的感觉；可以根据上下文加入轻微迟疑、笑意、叹气、呼吸、停顿、尾音变化和潜台词情绪，但不要每句话都堆满标注。每 1 到 3 句最多插入 1 个关键标注。
9. 如果没有任何可朗读对白，只输出：<EMPTY>。
