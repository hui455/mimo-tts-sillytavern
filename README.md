...
这些只是写法教程，不是设计音色，也不会出现在设计音色列表里。

整体风格标签建议放在最开头；唱歌用 `（唱歌）` 开头。方言和角色风格只作为 MiMo 的表达控制，不建议让 DeepSeek 大幅改写原对白。
...
音色设计提示词建议按官方教程写成 1-4 句，包含声线、情绪气质、语速/节奏、角色或场景。核心特征清楚比堆砌维度更重要；避免互相冲突的要求，也不要写混响、EQ、压缩等后期处理词。
插件内置的设计音色只保留女声预设。升级到该版本后，会把旧的自定义设计音色重置为内置女声列表，并清空已保存的声音克隆；语音缓存 key 也已升级，旧设计音色或旧克隆音色生成的缓存不会继续命中。

## 声音克隆

在 `新增声音克隆` 里填写显示名，上传 wav 或 mp3 样本后点击 `添加声音克隆`。添加后会出现在独立播放音色和 SillyTavern voice map 可选列表里。样本会随扩展设置保存在浏览器/SillyTavern 设置中，建议使用较短音频；插件会拒绝 base64 超过 10MB 的样本。当前迁移会清空旧克隆音色，需要时可重新添加。
