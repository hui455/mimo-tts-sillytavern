...
在 `新增声音克隆` 里填写显示名，上传 wav 或 mp3 样本后点击 `添加声音克隆`。添加后会出现在独立播放音色、第二人音色和 SillyTavern voice map 可选列表里。样本会随扩展设置保存在浏览器/SillyTavern 设置中，建议使用较短音频；插件会拒绝 base64 超过 10MB 的样本。当前迁移会清空旧克隆音色，需要时可重新添加。
